...
| `$NAS_MNT` | `/mnt/secretary-bot/ai-image`（Pi） / `N:\ai-image\`（Win） | NAS マウントポイント（画像生成側） |
...
| 3 | NAS 共有の相互可視性 | Pi bash: `ls $NAS_MNT/outputs` / Main PowerShell: `Get-ChildItem N:\ai-image\outputs` | 双方で同じ `YYYY-MM` フォルダが見える。片方で作ったテストファイルが他方から読める | §3.3 |
...
| 6 | NAS 成果物 | Pi bash: `ls $NAS_MNT/outputs/$(date +%Y-%m)/$(date +%Y-%m-%d)/` | 今の生成に対応する PNG が存在。Main からも `Get-ChildItem N:\ai-image\outputs\YYYY-MM\YYYY-MM-DD` で同ファイルが見える | §3.6 |
...
| 項目 | 内容 |
|---|---|
| 症状 | Pi では見えるが Windows からは `N:\` に何も見えない、片方で書いたものがもう片方で見えない、`Access denied` |
...
| 切り分け | Pi: `ls -la /mnt/secretary-bot && mount \| grep secretary-bot`。Main: `powershell: Get-SmbMapping; Get-ChildItem N:\ai-image\` / `Get-Acl N:\ai-image\outputs \| Format-List`。SYSTEM 実行 Agent からの可視性は `PsExec -s -i cmd.exe` で `net use` を確認 |
| 対処 | [`../nas_setup.md`](../nas_setup.md) §4 を再実行。Agent が SYSTEM で動く場合は「SYSTEM コンテキスト向けのマッピング」を `start_agent.bat` 先頭で `net use N: \\$NAS_HOST\secretary-bot ...` として冪等に叩く |
...
| 切り分け | Main PC: `powershell: "test" \| Out-File N:\ai-image\outputs\_w.txt`。Agent ログ `/system/logs?source=comfyui` の `SaveImage` 周辺。`image_jobs.result_paths` の値 | (a)(d) [`../nas_setup.md`](../nas_setup.md) §3/§4 で権限・ドライブレターを `N:` に統一。(b) `extra_model_paths.yaml` で `output_directory` を NAS に向ける。(c) `Get-SmbMapping` で再接続 |
...
# NAS マッピング確認
Get-SmbMapping
Get-ChildItem N:\ai-image\outputs
```
