  ...
  "files": [
    {
      "type": "checkpoints",
      "filename": "chenkinNoobXL_v05.safetensors",
      "nas_path": "//nas/ai-image/models/checkpoints/chenkinNoobXL_v05.safetensors",
      "sha256": "abc...",
      "size": 6738902345
    }
  ],
...
```json
{
  ...
  "inputs": {
    ...
    "output_dir": "//nas/ai-image/outputs/2026-04/2026-04-15"
  },
  "timeout_sec": 300,
  "required_models": [
    { "type": "checkpoints", "filename": "chenkinNoobXL_v05.safetensors" }
...
| `timeout_sec` | int | 任意 | 既定はプリセット依存。Agent 側でタイムアウト監視 |
...
完了時は `status="done"`、`result_paths` に NAS パス配列（例: `["//nas/ai-image/outputs/2026-04/2026-04-15/2026-04-15_123456_img_01H_1234567890.png"]`）。
...
| `preview` | `{ "b64_png": string, "step": int }`（ComfyUI の中間プレビュー、任意） |
...
```json
{
  ...
  "dataset_path": "//nas/ai-image/lora_datasets/project_a",
  "output_path": "//nas/ai-image/lora_work/project_a/checkpoints",
  "sample_prompts": [
    "1girl, solo, upper body, ...",
    "1girl, solo, full body, ...",
    "1girl, solo, bust up, ..."
  ],
  "tb_logdir": "//nas/ai-image/lora_work/project_a/logs"
}
```
...
```json
{
  ...
  "loss_avg": 0.082,
  "samples": [
    { "epoch": 3, "path": "//nas/ai-image/lora_work/project_a/samples/epoch3_01.png" }
  ],
  "checkpoints": [
    { "epoch": 3, "path": "//nas/ai-image/lora_work/project_a/checkpoints/epoch3.safetensors" }
  ],
...
curl -X POST -H "X-Agent-Token: $TOKEN" -H "Content-Type: application/json" \
  -d '{"files":[{"type":"checkpoints","filename":"chenkinNoobXL_v05.safetensors","nas_path":"//nas/ai-image/models/checkpoints/chenkinNoobXL_v05.safetensors","sha256":"abc..."}]}' \
