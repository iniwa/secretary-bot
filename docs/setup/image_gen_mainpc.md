...
> 関連: `docs/design/image_gen_design.md` / `image_gen_api.md` / `image_gen_nas_setup.md` / `image_gen_preset_compat.md`
>
> 本書は Claude Code がこのドキュメントを直接読みながら MainPC 実機で順番に実行することを想定している。各ステップはチェックボックス `- [ ]` で進捗管理し、検証コマンドの出力を確認してからユーザーに報告すること。
>
> **前提**: NAS 側の共有 `ai-image` は `docs/design/image_gen_nas_setup.md` の手順で初期化済みであること。未初期化なら先にそちらを完了させる。
...
- [ ] ChenkinNoob-XL-V0.5 の SDXL チェックポイントを入手。`[要ユーザー確認]`: **配布 URL はユーザーに指定してもらう**（Civitai 等、ログインが必要な配布元が多い）。ダウンロード先は NAS `<NAS>/ai-image/models/checkpoints/chenkinNoobXL_v05.safetensors`（ファイル名固定、`image_gen_preset_compat.md` §2.2 の要件）。
...
- [ ] VAE の要否を確認。`image_gen_preset_compat.md` §2.2 の通り SDXL 同梱 VAE で基本 OK。外部 VAE を使うプリセットを使用する場合は `<NAS>/ai-image/models/vae/` に配置する。`[要ユーザー確認]`: 外部 VAE 利用予定の有無をユーザーに聞く。
...
> **切り分け**: NAS 経由のコピーで 1GbE + HDD だと SDXL 7GB ≒ 1 分。極端に遅い（5 分以上）場合は NAS 側のディスク負荷 / SMB バージョンを確認（`image_gen_nas_setup.md` §8.1）。
...
- [ ] `N:` に割り当て（NAS 認証情報は `.env` と一致させる）。`[要ユーザー確認]`: 対話で資格入力する場合は `Get-Credential` を使う。
    ...
    ```powershell
    # 対話版（推奨）
    $cred = Get-Credential -UserName "<nas-user>" -Message "NAS (ai-image) credentials"
    New-SmbMapping -LocalPath "N:" -RemotePath "\\192.168.1.xx\ai-image" `
      -UserName $cred.UserName -Password $cred.GetNetworkCredential().Password `
      -Persistent $true

    # 非対話版（スクリプト内平文、非推奨）
    # net use N: \\192.168.1.xx\ai-image /user:<user> <pw> /persistent:yes
    ```

- [ ] マウント検証。`models/` と `outputs/` が見えること。

    ```powershell
    Get-ChildItem N:\
    Get-ChildItem N:\models
    Get-ChildItem N:\outputs -ErrorAction SilentlyContinue
    ```

    期待値: `models`, `outputs`, `lora_datasets`, `lora_work`, `workflows`, `snapshots` が並ぶ。見えない場合は NAS 側の共有初期化が未完了 → `docs/design/image_gen_nas_setup.md` §2 を実施。

- [ ] 書き込みテスト（MainPC は `outputs/`, `lora_work/`, `models/loras/` に書き込み可のはず）。

    ```powershell
    "test $(Get-Date -Format o)" | Out-File N:\outputs\_mainpc_write_test.txt
    Get-Content N:\outputs\_mainpc_write_test.txt
    Remove-Item N:\outputs\_mainpc_write_test.txt
    ```
...
- [ ] 起動ログで以下のキーを確認（順不同）。
    ...
    - `NAS connected: \\192.168.1.xx\ai-image`（`start_agent.bat` の最初）
    - `[Agent] Role: main`
...
- [ ] Sub PC セットアップ: `docs/setup/image_gen_subpc.md`（Main と差分は priority 値とモデル同期タイミングのみ想定）
- [ ] Pi セットアップ: `docs/setup/image_gen_pi.md`（Pi 側の `image_gen` / `workflow_mgr` ユニット設定、プリセット登録）
- [ ] `docs/design/image_gen_preset_compat.md` §4 に従い、Sub PC セットアップ完了後に MainPC と `/capability` を付き合わせる。差分があれば ComfyUI Manager Snapshot で同期。
