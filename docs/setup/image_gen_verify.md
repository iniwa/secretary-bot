...
> 関連:
> - [`../design/image_gen_design.md`](../design/image_gen_design.md) — 状態機械・エラー階層
> - [`../design/image_gen_api.md`](../design/image_gen_api.md) — API 仕様、エラーコード対応表
> - [`../design/image_gen_preset_compat.md`](../design/image_gen_preset_compat.md) — capability 照合
> - [`../design/image_gen_nas_setup.md`](../design/image_gen_nas_setup.md) — NAS セットアップ
> - [`image_gen_mainpc.md`](image_gen_mainpc.md) / [`image_gen_subpc.md`](image_gen_subpc.md) / [`image_gen_pi.md`](image_gen_pi.md) — 各ホストのセットアップ
...
`docs/design/image_gen_preset_compat.md` §3 の表を実行形式に落としたもの。`t2i_base` の前提崩れを検出する。
...
**空 diff** なら `t2i_base` 前提は満たされる。差分ありなら `docs/design/image_gen_preset_compat.md` §5 の「ロック or Snapshot 同期」手順へ。
...
| 対処 | (a) `POST /comfyui/setup` を叩くか [`image_gen_mainpc.md`](image_gen_mainpc.md) の初回セットアップ節へ。(b) §3.10。(c) ドライバ再インストール → 再起動。(d) `extra_model_paths.yaml` の `base_path` が `${SECRETARY_BOT_CACHE}/models/` を指しているか確認 |
...
| 対処 | [`../design/image_gen_nas_setup.md`](../design/image_gen_nas_setup.md) §4 を再実行。Agent が SYSTEM で動く場合は「SYSTEM コンテキスト向けのマッピング」を `start_agent.bat` 先頭で `net use Z: \\$NAS_HOST\ai-image ...` として冪等に叩く |
...
| `warming_cache` で停滞 | NAS → Agent SSD コピー失敗・遅延、sha256 不一致 | `curl -N -H "X-Agent-Token:$T" http://<agent>:7777/cache/sync/<sync_id>/stream` を購読。`image_jobs.cache_sync_id` 参照 | sha256 不一致なら NAS 側サイドカー再生成（[`../design/image_gen_nas_setup.md`](../design/image_gen_nas_setup.md) §5）。10 分超で自動 `failed`、retry 余地があれば `queued` 戻り |
...
| 切り分け | Main PC: `powershell: "test" \| Out-File Z:\outputs\_w.txt`。Agent ログ `/system/logs?source=comfyui` の `SaveImage` 周辺。`image_jobs.result_paths` の値 | (a)(d) [`../design/image_gen_nas_setup.md`](../design/image_gen_nas_setup.md) §3/§4 で権限・ドライブレターを `Z:` に統一。(b) `extra_model_paths.yaml` で `output_directory` を NAS に向ける。(c) `Get-SmbMapping` で再接続 |
...
| 対処 | [`../design/image_gen_preset_compat.md`](../design/image_gen_preset_compat.md) §5.2 の Snapshot 同期手順。`t2i_base` のみであれば custom_nodes 非依存なので無視して可。他プリセットは一時的に `workflows.main_pc_only=1` に更新して隔離:<br>`sqlite3 ... "UPDATE workflows SET main_pc_only=1 WHERE name='...';"` |
...
`image_jobs.last_error` は Agent が返した §1.4 エラー JSON 文字列（そのまま格納）。キーは `error_class` / `message` / `retryable` / `detail`。`error_class` でリトライ方針が決まる（`../design/image_gen_api.md` §10）。
...
Agent 側 SSE（`/image/jobs/{id}/stream`）は `progress`/`log`/`preview`/`status`/`result`/`error`/`done` でより詳細（`../design/image_gen_api.md` §4.3）。
