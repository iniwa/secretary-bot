...
| PC | 役割 | IP | 担当ガイド |
|---|---|---|---|
| **Raspberry Pi** | Bot 本体・Dispatcher・WebGUI | — | [image_gen_pi.md](image_gen_pi.md) |
| **MainPC** | ComfyUI / kohya 実行・画像生成の主系 | 192.168.1.210 | [image_gen_mainpc.md](image_gen_mainpc.md) |
| **SubPC** | ComfyUI / kohya 実行・負荷分散 + LoRA 副系 | 192.168.1.211 | [image_gen_subpc.md](image_gen_subpc.md) |
...
| # | 作業 | 担当 PC | 参照 |
|---|---|---|---|
| 1 | NAS 初期ディレクトリ作成・権限・モデル正本配置 | 任意（Pi か MainPC） | [image_gen_nas_setup.md](../design/image_gen_nas_setup.md) |
| 2 | MainPC セットアップ（ComfyUI / kohya / Agent 起動） | MainPC | [image_gen_mainpc.md](image_gen_mainpc.md) |
| 3 | SubPC セットアップ（MainPC 完了後に差分実行） | SubPC | [image_gen_subpc.md](image_gen_subpc.md) |
| 4 | Pi セットアップ（bot の image_gen 有効化） | Pi | [image_gen_pi.md](image_gen_pi.md) |
| 5 | E2E 動作確認 | Pi から | [image_gen_verify.md](image_gen_verify.md) |
...
### MainPC / SubPC
```
docs/setup/image_gen_mainpc.md （または _subpc.md）を見て、記載の手順を順番に実行してください。
...
### Pi
```
docs/setup/image_gen_pi.md を見て、記載の手順を順番に実行してください。
必要に応じて `docs/design/image_gen_nas_setup.md` の Pi 側手順を併用してください。
Portainer の操作とコード更新を含むので、[要ユーザー確認] タグでは必ず確認を取ってください。
```

### 全体動作確認
```
docs/setup/image_gen_verify.md の §1 E2E 7 段階チェック表を上から実行し、
...
| ドキュメント | 用途 |
|---|---|
| [image_gen_design.md](../design/image_gen_design.md) | 設計全体・状態機械・エラー階層 |
| [image_gen_api.md](../design/image_gen_api.md) | Windows Agent API 仕様（20 エンドポイント） |
| [image_gen_nas_setup.md](../design/image_gen_nas_setup.md) | NAS 初期化・マウント手順 |
| [image_gen_preset_compat.md](../design/image_gen_preset_compat.md) | Main/Sub capability 差分許容範囲 |
...
Phase 1 未対応項目の一覧は [image_gen_verify.md §6 既知の制約](image_gen_verify.md) を参照。
...
- [ ] `/mnt/ai-image/outputs/YYYY-MM/YYYY-MM-DD/` に PNG と `.sha256` サイドカーが保存される
