...
> 前提: **`docs/setup/image_gen_mainpc.md` が完了していること**（本書は差分中心）。共通手順は MainPC ガイド側の節番号を参照する。
> 関連: [`../design/image_gen_design.md`](../design/image_gen_design.md), [`../design/image_gen_preset_compat.md`](../design/image_gen_preset_compat.md), [`../design/image_gen_nas_setup.md`](../design/image_gen_nas_setup.md), [`../design/image_gen_api.md`](../design/image_gen_api.md)
...
- [ ] Windows 11 が最新で、NVIDIA ドライバも MainPC と**同系列のバージョン**に揃っている（ドライバ版差は `image_gen_preset_compat.md` §6 で微小差の原因になる）
...
SubPC の custom_nodes は **MainPC を正本として追従**する。`image_gen_preset_compat.md` §5.2 の方針に従う。
...
- Pi からの `POST /comfyui/update`（`image_gen_api.md` 予定仕様）で NAS 上の最新 snapshot を SubPC が自動取り込みする想定。現状は **手動同期**でよい。
...
手順は MainPC と完全に同一。ドライブレターも `N:` で統一する。詳細は [`../design/image_gen_nas_setup.md`](../design/image_gen_nas_setup.md) §4.2 を参照。
...
`image_gen_preset_compat.md` §3 の差分チェック表に沿って比較する。
...
- [ ] 代表プリセット `t2i_base` を Pi Dispatcher 経由で SubPC に流した際、OOM なく完了することを軽く確認（Phase 1 完了後の知覚的同等性検証は `image_gen_preset_compat.md` §4）
...
- [ ] ComfyUI の `--lowvram` や tile VAE は最終手段（`image_gen_preset_compat.md` §6 のとおり、VAE タイリング差は画素差の原因になる）
...
- [ ] それでも合わない（依存衝突）なら当該プリセットを `main_pc_only=1` で隔離（`image_gen_preset_compat.md` §5.1）

---

## 12. 次ステップ

- Pi 側ディスパッチャの登録と AgentPool priority 設定 → **Pi セットアップガイド**（別途作成予定、`docs/setup/image_gen_pi.md`）
- 互換性検証（seed 固定・SSIM チェック）は Phase 1 完了後に [`../design/image_gen_preset_compat.md`](../design/image_gen_preset_compat.md) §4 に従って実施
