...
### A-1. `windows-agent/agent.py` の `/activity` 拡張 ✅ 実装済み
...
### A-2. DB スキーマ migration（`src/database.py`） ✅ 実装済み
...
### A-3. `src/activity/collector.py` を Main/Sub 両対応に ✅ 実装済み
...
### A-4. `src/activity/detector.py` の `get_status()` 拡張 ✅ 実装済み
...
### B-1. Input-Relay sender の変更（外部リポ `C:/Users/yamatoishida/Documents/git/input-relay`） ✅ 実装済み

> 外部リポ側で変更完了。コミット＆ submodule ポインタ更新は別途実施する必要あり（ユーザー側作業）:
> ```
> cd C:/Users/yamatoishida/Documents/git/input-relay && git add -A && git commit -m "feat: expose input source timestamps for multi-PC activity detection"
> cd C:/Users/yamatoishida/Documents/git/secretary-bot && git submodule update --remote windows-agent/tools/input-relay
> ```
...
### B-2. `windows-agent/agent.py` から sender 状態を取得 ✅ 実装済み
...
### B-3. `src/activity/detector.py` に active_pcs 判定を追加 ✅ 実装済み
...
### B-4. `src/activity/collector.py` で active_pcs を記録 ✅ 実装済み
...
### C-1. `src/web/app.py` の `/api/activity` 拡張 ✅ 実装済み

> `/api/activity/stats` で foreground を pc 別にグルーピング。同時操作時間を `simultaneous_sec` で返す。
...
### C-4. `src/inner_mind/context_sources/activity.py` ✅ 実装済み
