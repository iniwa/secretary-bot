...
> 関連: [`image_gen_design.md`](image_gen_design.md) §ファイル配置・キャッシュ戦略（案 B）／§プリセット可搬性、[`image_gen_api.md`](image_gen_api.md) §2.1 `GET /capability`
> 版: 2026-04-15（初版）
...
3. **記録**
   - 検証結果は `docs/design/image_gen_preset_compat.md`（本書）に追記するか、別途 `docs/design/image_gen_preset_compat_results.md` を設けて日付・版ごとに残す。
...
`image_gen_design.md` §プリセット可搬性／カスタムノード同期（2 段構え）に従い、以下を運用する。
